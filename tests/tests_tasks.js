...
test('Task - GET /tasks/', function () {
    ...
    deepEqual(tasks_get(), {tasks: tasks[0].slice(0,3).concat(tasks[1]).concat(
        tasks[0].slice(3,8))},
        '\'priority\' Query - Mixed priorities; all returned.');
    deepEqual(tasks_get_filter('priority=-1'),
        {tasks: [tasks[0][6], tasks[0][7]]},
        '\'priority\' Query - Low priorities.');
    deepEqual(tasks_get_filter('priority=1'),
        {tasks: [tasks[0][4], tasks[0][5]]},
        '\'priority\' Query - High priorities.');
    deepEqual(tasks_get_filter('priority=0'),{tasks: 
        tasks[0].slice(0,3).concat(tasks[1]).concat(tasks[0].slice(3,4))},
        '\'priority\' Query - Normal priorities.');

    // Test Filtering Params - Completed
    tasks[0].push(task_create(
        {title: 'Completed Task', list: lists[0]._id, completed: true}).task);

    deepEqual(tasks_get_filter('completed=1'), {tasks: [tasks[0][8]]},
        '\'completed\' Query - Completed Tasks');

    // Test Filtering Params - Dates
    tasks[0].push(task_create({title: 'Dated 1-19-1970',
        list: lists[0]._id, deadline: new Date(1970, 1, 19)}).task);
    tasks[0].push(task_create({title: 'Dated 1-20-1970',
        list: lists[0]._id, deadline: new Date(1970, 1, 20)}).task);
    tasks[0].push(task_create({title: 'Dated 1-20-1970',
        list: lists[0]._id, deadline: new Date(1970, 1, 20)}).task);
    tasks[0].push(task_create({title: 'Dated 1-21-1970',
        list: lists[0]._id, deadline: new Date(1970, 1, 21)}).task);
    tasks[0].push(task_create({title: 'Dated 1-22-1970',
        list: lists[0]._id, deadline: new Date(1970, 1, 22)}).task);
    tasks[0].push(task_create({title: 'Dated 1-22-2008',
        list: lists[0]._id, deadline: new Date(2008, 1, 22)}).task);

    deepEqual(tasks_get_filter('endDate=' + datestring(1, 20, 1970) +
