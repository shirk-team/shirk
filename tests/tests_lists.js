...
test('List - GET /lists/:id', function () {
    ...
    deepEqual(list_get(lists[0]._id), {list: lists[0], tasks: tasks[0]},
        'Non-empty List ' + lists[0]._id.toString() + ' retrieved with ' +
        tasks[0].length.toString() +' unque Tasks.');
    deepEqual(list_get(lists[1]._id), {list: lists[1], tasks: tasks[1]},
        'Non-empty List ' + lists[1]._id.toString() + ' retrieved with ' +
        tasks[1].length.toString() +' unque Tasks.');

    // Test Filtering Params - Limit
    deepEqual(list_get_filter(lists[0]._id, 'limit=2'),
        {list: lists[0], tasks: tasks[0].slice(0,2)},
        '\'limit\' Query - Correct number returned.');

    // Test Filtering Params - Priority
    tasks[0].push(task_create(
        {title: 'Priority Normal', list: lists[0]._id, priority: 0}).task);
    tasks[0].push(task_create(
        {title: 'Priority High 1', list: lists[0]._id, priority: 1}).task);
    tasks[0].push(task_create(
        {title: 'Priority High 2', list: lists[0]._id, priority: 1}).task);
    tasks[0].push(task_create(
        {title: 'Priority Low 1', list: lists[0]._id, priority: -1}).task);
    tasks[0].push(task_create(
        {title: 'Priority Low 2', list: lists[0]._id, priority: -1}).task);

    deepEqual(list_get(lists[0]._id), {list: lists[0], tasks: tasks[0]},
        '\'priority\' Query - Mixed priorities; all returned.');
    deepEqual(list_get_filter(lists[0]._id, 'priority=-1'),
        {list: lists[0], tasks: [tasks[0][6], tasks[0][7]]},
        '\'priority\' Query - Low priorities.');
    deepEqual(list_get_filter(lists[0]._id, 'priority=1'),
        {list: lists[0], tasks: [tasks[0][4], tasks[0][5]]},
        '\'priority\' Query - High priorities.');
    deepEqual(list_get_filter(lists[0]._id, 'priority=0'),
        {list: lists[0], tasks: tasks[0].slice(0,4)},
        '\'priority\' Query - Normal priorities.');

    // Test Filtering Params - Completed
    tasks[0].push(task_create(
        {title: 'Completed Task', list: lists[0]._id, completed: true}));
    deepEqual(list_get_filter(lists[0]._id, 'completed=1'),
        {list: lists[0], tasks: [tasks[0][8].task]},
        '\'completed\' Query - Completed Tasks');

    // Test Filtering Params - Dates
    tasks[0].push(task_create({title: 'Dated 1-19-1970', list: lists[0]._id,
        deadline: new Date(1970, 1, 19)}).task);
    tasks[0].push(task_create({title: 'Dated 1-20-1970', list: lists[0]._id,
        deadline: new Date(1970, 1, 20)}).task);
    tasks[0].push(task_create({title: 'Dated 1-20-1970', list: lists[0]._id,
        deadline: new Date(1970, 1, 20)}).task);
    tasks[0].push(task_create({title: 'Dated 1-21-1970', list: lists[0]._id,
        deadline: new Date(1970, 1, 21)}).task);
    tasks[0].push(task_create({title: 'Dated 1-22-1970', list: lists[0]._id,
        deadline: new Date(1970, 1, 22)}).task);
    tasks[0].push(task_create({title: 'Dated 1-22-2008', list: lists[0]._id,
        deadline: new Date(2008, 1, 22)}).task);

    deepEqual(list_get_filter(lists[0]._id, 'endDate=' + datestring(1, 20, 1970)
        + '&startDate=' + datestring(1, 20, 1970)),
    ...
    deepEqual(list_get_filter(lists[0]._id,
        'startDate=' + datestring(1, 20, 1970) + '&endDate=' +
        datestring(1,21,1970)),
